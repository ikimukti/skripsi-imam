...
# Generate data_pelanggaran and data_prestasi
data_pelanggaran = []
data_prestasi = []

for siswa_id in siswa_ids:
    # Generate pelanggaran data for 40% of students
    ...
    # Generate prestasi data for 30% of students
    if random.random() < 0.3:
        ...
        for prestasi_id in prestasi_sample:
            today = datetime.datetime.now()
            start_date = today - datetime.timedelta(days=365 * 3)  # 3 Years ago
            end_date = today
            tanggal = fake.date_time_between(start_date=start_date, end_date=end_date)
            jam = fake.date_time_between(start_date=tanggal, end_date=tanggal.replace(hour=16))
